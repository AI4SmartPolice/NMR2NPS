...
data_test = pd.read_excel(excel_file, sheet_name='mix_test', header=0)
...
def train_evaluate_model(X_train_nmr, y_train, X_train_smiles, y_train_smiles, X_test_nmr, y_test, dataset_name, method, label_mapping, id_sample_test, seed):
    ...
    pred_df.to_csv(f'pro_cnn_cl_mix_predictions/predictions_{method}_seed_{seed}.csv', index=False)
